			...
			<div class="card-body">
				<form method="POST" action="<?= base_url('admin/transaksi/cek_pembayaran'); ?>">
					<?php foreach ($pembayaran as $item) : ?>
						<a href="<?= base_url('admin/transaksi/download_pembayaran/' . $item->id_rental); ?>" class="btn btn-sm btn-success">
							<i class="fas fa-download"></i> Download Bukti Pembayaran
						</a>
						<div class="custom-control custom-switch ml-5">
							<input type="hidden" class="custom-control-input" value="<?= $item->id_rental ?>" name="id_rental">
